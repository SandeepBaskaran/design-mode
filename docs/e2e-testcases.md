...
| 3.23a | Appearance: Corner shape | Select an element with a non-zero border-radius → in the Opacity/Corner-radius row, click the **corner-shape icon** (between the radius field and the `scan` expand icon) → pick `squircle` (or bevel/scoop/notch) from the popover | The popover shows six shape swatches (round active by default) with labels; picking one applies `corner-shape` and closes. On newest Chromium the corners reshape live; the change appears in the Changes tab under Appearance and in Export CSS. Reselecting shows the trigger reflecting the set shape. Clicking outside closes the popover. On browsers without support the property is recorded but has no visual effect |
