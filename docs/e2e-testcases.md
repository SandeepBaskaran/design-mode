...
| 10.6 | Text change format — inline diff | Edit part of a paragraph's text | Bullet reads `- {label} text: {inline diff}` where only the changed words show — `~~removed~~`, `**added**`, and `…` for elided unchanged runs (a few words of context kept each side). A `## Changes` legend explains the notation once. A total rewrite (no shared words) or an oversized edit falls back to `text → "{new}"` |
