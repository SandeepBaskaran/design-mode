...
| 3.23a | Appearance: Corner shape | Select an element with a non-zero border-radius → Appearance → Advanced → set **Corner shape** to `squircle` (or bevel/scoop/notch) | On newest Chromium the corners reshape live (squircle = iOS-style, bevel = flat cut, etc.); the `corner-shape` change appears in the Changes tab under Appearance and in Export CSS. Reselecting the element shows the dropdown reflecting the set value. On browsers without support the property is recorded but has no visual effect |
