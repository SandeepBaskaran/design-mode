...
async function boot() {
  ...
  try {
    ...
    log(`  ${dim('\u2022')} get_changes        ${dim('\u2014 Get all CSS changes with ready-to-use CSS block')}`);
    log(`  ${dim('\u2022')} get_comments        ${dim('\u2014 Get comments/annotations on DOM elements')}`);
    log(`  ${dim('\u2022')} get_session_summary ${dim('\u2014 Session stats: changes, elements, connection')}`);
    log(`  ${dim('\u2022')} apply_changes       ${dim('\u2014 Push CSS changes to browser for live preview')}`);
    log(`  ${dim('\u2022')} clear_changes       ${dim('\u2014 Reset session and start fresh')}`);
    log(`  ${dim('\u2022')} get_tailwind        ${dim('\u2014 Export changes as Tailwind utility classes')}`);
    log(`  ${dim('\u2022')} get_scss            ${dim('\u2014 Export as nested SCSS')}`);
    log(`  ${dim('\u2022')} get_jsx_styles      ${dim('\u2014 Export as camelCase JSX inline style objects')}`);
    log('');
    log(`==================================================`);
