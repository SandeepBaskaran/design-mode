...
- **Build the extension:** `node build.mjs` from
  `packages/extension/` (NOT from the repo root — `build.mjs` lives
  inside the package).
...
- **Repo-wide gate:** `npm run verify` from repo root — runs
  ...
  bundle integrity → manifest sanity → MCP local checks → build
  local MCP → bundle integrity → build website → export integrity.
  CI uses the same command.
- **Chrome Web Store zip filename is always
  `design-mode-extension.zip`.** Replace the existing zip; never
  produce per-version copies like `design-mode-v1.0.2.zip`. The file
  lives at `packages/extension/design-mode-extension.zip` and is
  generated by `npm run package:extension`.
...
- **Release workflow** fires on `v*.*.*` tag push: builds the
  extension → zips `design-mode-extension.zip` → creates a GitHub
  Release with the zip attached. The tag push is the maintainer's
  explicit consent; the workflow only fires after it.
