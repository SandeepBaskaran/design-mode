...
| **Corner shape** (icon dropdown) | `corner-shape` | Reshapes the corners that `border-radius` rounds — the CSS equivalent of Figma's corner smoothing. The trigger shows a live preview of the current shape; clicking expands a labelled list (swatch + name per row, current one checked) inline below the row. Only visible with a non-zero `border-radius`; newest Chromium only (harmlessly ignored elsewhere, where the swatches fall back to rounded but the labels stay). | `round` (default), `squircle` (iOS-style superellipse), `square` (sharp), `bevel` (flat diagonal cut), `scoop` (concave inward curve), `notch` (90° inward cutout). |
